...
CREATE TABLE IF NOT EXISTS lead_documents (
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_docs_lead FOREIGN KEY (id) REFERENCES leads(u_id) ON DELETE CASCADE,
  KEY idx_lead_doc (id, doc_type)
) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS runs (
  ...
  CONSTRAINT fk_runs_lead FOREIGN KEY (id) REFERENCES leads(u_id) ON DELETE CASCADE,
  KEY idx_runs_lead (id),
...
This file holds configuration for the frontend. It contains two URLs: one for the n8n webhook and one for our local document server.
